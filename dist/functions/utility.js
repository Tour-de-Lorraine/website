    ...
    if (typeof input === 'object') {
        ...
        // Copy Object
            ...
            for (const _key in input)
                newObject[_key] = deepCopy(input[_key]);
            return newObject;
        }
    }
...
function getViewportWidth(lastW = '') {
    let w;
    if (innerWidth < layoutBreakpoints.s[1])
        w = 's';
    else if (innerWidth < layoutBreakpoints.m[1])
        w = 'm';
    else if (innerWidth < layoutBreakpoints.l[1])
        w = 'l';
    else
        w = 'xl';
    if (lastW === w)
