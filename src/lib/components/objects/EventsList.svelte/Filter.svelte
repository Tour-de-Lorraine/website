<script>
	import event from '$lib/directus/queries/event.js';
	import {formatDate} from '$lib/functions/formatDateTime.js';
...
<style lang="scss">
	.FILTER {
		...
		position: sticky;
		top: calc(var(--header-height) + 0.75px); // <- Fix pixel rounding error
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
	@media (width <= $bp-s-maxWidth) {
		.FILTER {
			...
			input._toggleFilter:checked ~ form {
				display: flex;
			}
		}
	}
